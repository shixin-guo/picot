...
fn scheduled_task_from_row(row: &Row) -> rusqlite::Result<ScheduledTask> {
    ...
    Ok(ScheduledTask {
        ...
        model: row.get("model")?,
        frequency: TaskFrequency::parse(&frequency).map_err(|_| rusqlite::Error::InvalidQuery)?,
...
mod tests {
    use super::{
        advance_next_run_at, now_unix, MetadataStore, ScheduledTaskPatch, TaskFrequency, TaskStatus,
    };
    use std::fs;
    ...
    fn failed_runs_increment_retry_count_and_catch_up_selects_capped_failures() {
        ...
        store.reset_scheduled_task_for_manual_run(&task.id).unwrap();
