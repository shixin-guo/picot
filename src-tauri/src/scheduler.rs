...
async fn run_catch_up(deps: &SchedulerDeps) {
    let tasks = {
        ...
        store
            .catch_up_scheduled_tasks(now_unix(), RETRY_CAP)
            .unwrap_or_default()
    };
    for task in tasks {
...
pub async fn execute_task(deps: &SchedulerDeps, task: ScheduledTask) {
    ...
    if let Ok(mut store) = deps.metadata.lock() {
        let _ = store.mark_scheduled_task_finished(&task.id, success, session_id.as_deref());
    }
...
fn notify_completion(
    ...
    if let Err(error) = crate::show_completion_notification(app, title, &body, cwd, session_id) {
