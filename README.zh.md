...
[从 GitHub Releases 下载](https://github.com/shixin-guo/pi-studio/releases)

**无需单独安装 `pi` CLI** — Pi Studio 内置了自己的 pi 运行时。

### macOS 未签名提示

Pi Studio 目前发布的 macOS 版本未经 Apple 开发者 ID 签名/公证，系统可能弹出：

`"Pi Studio" 无法打开，因为无法验证开发者。`

**解决方法：**

1. 将 `Pi Studio.app` 拖入 `/Applications`
2. 右键点击 → **打开**
...
Pi Studio 为 Pi 提供完整的可视化界面。打开任意项目文件夹，与 Agent 对话，浏览会话和文件——无需打开终端。多个项目可以并行运行，每个项目有独立窗口和独立 Agent 进程。
...
- 项目头部显示**当前 Git 分支**
- **在外部编辑器中打开** — 直接从 Pi Studio 启动 VS Code、Cursor 等
- 原生文件夹选择器，无需使用终端打开项目
...
Pi Studio 不重新实现 Agent 逻辑——它内嵌 Pi 并通过原生 UI 暴露其运行时能力。
...
嵌入的 pi 进程启动时加载 `embedded-server.mjs`。该扩展负责 Tauri WebView 所通信的 HTTP + WebSocket 层：静态资源、`/api/sessions`、`/api/cost-dashboard`、提示词 RPC 桥接等。Pi Studio 的 Rust 层负责进程生命周期、端口分配和窗口管理。
...
通过任意工作区内的 `pi /login` 提供模型凭证，或直接写入 `~/.pi/agent/auth.json`。Pi Studio 本身不管理凭证。
...
Pi Studio 是 **Tau** 的维护性 fork，专为 Pi 优先的本地开发工作流定制。主要增强：

- **Tauri 原生 PiManager** — 每个项目窗口启动一个独立的 `pi --mode rpc` 进程
- **内嵌 pi 运行时** — 无需全局安装，Pi Studio 自带二进制
