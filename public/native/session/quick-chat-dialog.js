...
// ABOUTME: Adapted from feature-v3's quick-chat-dialog.js — the transport
// ABOUTME: (broker) is replaced with the native host protocol via hostRequest
// ABOUTME: and the runtime is an EphemeralChatRuntime fed by runtime.subscribe.
...
export class QuickChatDialog {
  ...
  async open() {
    ...
    this._show();
    try {
      this.descriptor = await this._create();
  ...
  minimize() {
    ...
    this.ephemeralRuntime.active = false;
  ...
  async replace() {
    ...
    if (this.descriptor) {
      const risk = this.ephemeralRuntime?.getCloseRisk();
    ...
    this._locked = true;
    try {
      const next = await this._replace();
      if (!next) return null;
  ...
  getCloseRisk() {
    if (!this.ephemeralRuntime) return null;
    return this.ephemeralRuntime.getCloseRisk();
  }
  ...
  cleanupAfterHostClose() {
    ...
    if (this.ephemeralRuntime?.isStreaming) this.ephemeralRuntime.abort();
    this._disposeRuntime();
    this.descriptor = null;
    this._dialog?.classList.add("hidden");
    this._chip?.classList.add("hidden");
  ...
  _buildDom(doc) {
    ...
    this._onDialogKeyDown = (event) => {
      if (event.key !== "Escape" || !this.ephemeralRuntime?.isStreaming) return;
      event.preventDefault();
      event.stopPropagation();
      this.ephemeralRuntime.abort();
    };
  ...
  _mount() {
    this.ephemeralRuntime = this._bindRuntime(this.descriptor);
    this.ephemeralRuntime.active = true;
    this._onRuntimeRenderState = () => this._renderChipState();
    this._onRuntimeUnread = () => this._renderChipState();
    this.ephemeralRuntime.addEventListener("renderstate", this._onRuntimeRenderState);
    this.ephemeralRuntime.addEventListener("unreadchange", this._onRuntimeUnread);
    this.view = this.createView(this.ephemeralRuntime);
    if (this.view?.element) {
      this._body.replaceChildren(this.view.element);
    }
    this._dialog.setAttribute("aria-busy", "false");
    this._renderChipState();
    this.ephemeralRuntime.requestSnapshot();
  }
  ...
  _show() {
    ...
    this._chip.classList.add("hidden");
    this.view?.activate?.();
    this.ephemeralRuntime?.acknowledgeVisible();
    if (!this._positioned) {
      this._center();
  ...
  _renderChipState() {
    if (!this._chip || !this.ephemeralRuntime) return;
    const labels = [t("ephemeral.quickChat")];
    if (this.ephemeralRuntime.isStreaming) labels.push(t("ephemeral.generating"));
    if (this.ephemeralRuntime.unread) labels.push(t("ephemeral.unread"));
    this._chip.textContent = labels.join(" · ");
    this._chip.setAttribute("aria-label", labels.join(" · "));
  }

  _disposeRuntime() {
    if (this.ephemeralRuntime && this._onRuntimeRenderState) {
      this.ephemeralRuntime.removeEventListener("renderstate", this._onRuntimeRenderState);
    }
    if (this.ephemeralRuntime && this._onRuntimeUnread) {
      this.ephemeralRuntime.removeEventListener("unreadchange", this._onRuntimeUnread);
    }
    this.ephemeralRuntime?.destroy();
    this.view?.destroy?.();
    this.unsubscribe?.();
    this.unsubscribe = null;
