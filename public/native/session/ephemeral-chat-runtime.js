...
export class EphemeralChatRuntime extends EventTarget {
  constructor({ descriptor, runtime, target }) {
    super();
  ...
  _send(payload) {
    ...
    if (!this.runtime || !this.target) {
      this._emitFailure(new Error("ephemeral runtime unavailable"));
      return null;
    }
    const requestId = `eph-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    this._pendingCommandIds.add(requestId);
    this.runtime
      .request(payload, this.target, { idempotencyKey: requestId })
      .then((frame) => {
  ...
  _request(payload) {
    return new Promise((resolve, reject) => {
      ...
      if (!requestId) {
        reject(new Error("ephemeral runtime unavailable"));
        return;
      }
