...
- **HTTP REST** 在 `/api/*` 下 —— `/api/health`、`/api/sessions`、
  `/api/files`、`/api/search`、`/api/cost-dashboard`、`/api/lan-qr`、
  `/api/instances`、`/api/open`、`/api/agent-config`、`/api/models-config`、
  `/api/git-branch`，加 `POST /api/rpc` 透传。`public/` 下的静态资源在
  `/` 下分发。
